...
static uint32_t
dc_tx_first_req(struct dc_tx *tx, bool write)
{
	if (tx->tx_flags & DAOS_TF_RDONLY)
...
dc_tx_commit_cb(tse_task_t *task, void *data)
{
	...
	if (tx->tx_pm_ver < oco->oco_map_version) {
		...
		dcsr = &tx->tx_req_cache[dc_tx_first_req(tx, false)];
...
dc_tx_commit_prepare(struct dc_tx *tx, tse_task_t *task)
{
	...
	start = dc_tx_first_req(tx, false);
	...
	for (i = 0; i < req_cnt; i++) {
		...
		if (dcsr->dcsr_opc == DCSO_PUNCH_OBJ) {
			...
			rc = dc_tx_classify_common(tx, dcsr, dtrgs, tgt_cnt,
					grp_idx, i, dcsr->dcsr_opc == DCSO_READ,
					false, &leader_dtrg_idx, &act_tgt_cnt,
					&dtr_list, &leader_oid);
			if (rc != 0)
				goto out;
		}
